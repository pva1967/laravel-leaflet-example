...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                @if (Request::is('admin/*'))
                <form method="POST" action="{{ route('admin.cont2outlets.store', $outlet) }}" accept-charset="UTF-8">
                @else
                <form method="POST" action="{{ route('cont2outlets.store', $outlet) }}" accept-charset="UTF-8">
                @endif
                    {{ csrf_field() }}
                <table class="table table-sm table-responsive-sm">
                    ...
                    @foreach($contacts as $key => $contact)
                        <tr>
                            <td class="text-center"><input type="checkbox" name="contact[]" {{in_array($contact->id, $contact_locs)?'checked':''}} value="{{$contact->id}}"> </td>
                            <td>{{$contact->name}}</td>
                            <td>{{ $contact->email }}</td>
                            <td>{{ $contact->phone }}</td>
